...
function main() {
  ...
  if (!semver.valid(version)) {
    throw new Error(
      `Invalid VERSION. Expected semver like X.Y.Z (optionally prefixed by release/, refs/tags/, or v). Got: ${rawVersion}`,
    );
  }
  ...
  // Make sure we have all tags/refs
  ...
  console.log(
    `Creating branch ${branch} from ${releaseTag} (${tagCommitRes.out})`,
  );
